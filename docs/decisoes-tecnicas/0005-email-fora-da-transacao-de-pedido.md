...
## Consequências
- Não há fila de retry automática para emails que falham — só log de warning (ver `docs/ferramentas/smtp-gmail.md`). Se isso se tornar um problema real em produção, considerar uma fila (ex.: tabela de "notificações pendentes" + job, ou um message broker).
