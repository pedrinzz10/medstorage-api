...
4. **Email deve ser enviado ao marcar ATENDIDO.** Implementado via Gmail SMTP (`spring-boot-starter-mail`). O envio ocorre em `TransactionSynchronizationManager.afterCommit()` — só dispara após a transação confirmar, garantindo que o pedido já está salvo. Falha de SMTP é capturada e logada (`log.warn`) sem propagar exceção, logo não bloqueia o fluxo de status.
...
| Email falha ao enviar | Log warn + não bloqueia o fluxo de status (sem retry automático no MVP) |
